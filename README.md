...
- 최용성: git을 이용한 협업을 통해 pr방법에 대해서 더 자세하게 배우는 시간이 되었던것같다. 또한 sql 쿼리문을 짜거나 jdbc를 이용한 DB연결과 같은 작업을 통해 새롭게 알게 된 사실이 많았다.

- 나윤하: 기본기가 부족해 프로젝트 시작 전 두려움이 컸지만, 팀원들의 도움으로 작은 역할이라도 수행하며 끝까지 참여할 수 있었다.
GitHub 협업 과정과 Eclipse 구조를 실무적으로 경험하며 많은 것을 배웠고, 앞으로 기본기를 더 탄탄히 다져야겠다고 느꼈다.

- 이상민: 이 과제를 하면서 부족했던 점을 복습할 수 있어서 좋았습니다. 
함께 하는 팀원분들이 수고해주셔서 성공적으로 마무리 할 수 있었습니다. 
앞으로도 부족한 점은 배우고 성장하는 사람이 되고 싶습니다.

- 김진필: 만화책 대여점 CLI 프로젝트에서 만화책 관리 기능과 전체 흐름(App, Repository, JDBC 연동)을 구현하며 Soft Delete 방식까지 적용헤보며 프로그램 구조를 이해할 수 있었다. Git 기반 브랜치 협업을 통해 기능별 개발, 테스트, merge 과정을 직접 경험하며 팀 코드 통합과 문제 해결 능력을 키울 수 있었다.
